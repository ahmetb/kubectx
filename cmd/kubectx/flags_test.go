...
func Test_parseArgs_new(t *testing.T) {
	tests := []struct {
		...
		{name: "tmp switch by name",
			args: []string{"--tmp", filepath.Join(os.TempDir(), "kubectx-test"), "foo"},
			want: TmpOp{Inner: SwitchOp{Target: "foo"}, Value: filepath.Join(os.TempDir(), "kubectx-test")}},
		{name: "tmp separated path",
			args: []string{"--tmp", filepath.Join(os.TempDir(), "kubectx-test")},
			want: TmpOp{Inner: ListOp{}, Value: filepath.Join(os.TempDir(), "kubectx-test")}},
		{name: "tmp delete",
			args: []string{"-d", "--tmp", filepath.Join(os.TempDir(), "kubectx-test"), "."},
			want: TmpOp{Inner: DeleteOp{[]string{"."}}, Value: filepath.Join(os.TempDir(), "kubectx-test")}},
